...
void	cmd_ld(unsigned char *board, t_proc *c_proc)
{
	...
	if (c_proc->ctp == 5)
	{
		if (board[(c_proc->i + 1) % MEM_SIZE] == 0x90)
		{
				...
				//PROB AVEC LES DIRECT ...

				c_proc->r[(int)(board[c_proc->i + 6 % MEM_SIZE]) - 1] = bit_cat(board, c_proc, (id % IDX_MOD), 4);
				
				// ft_printf("VERIF REG : %x\n", c_proc->r[(int)(board[c_proc->i + 6 % MEM_SIZE]) - 1]);
