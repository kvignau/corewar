...
static void				add_dir_reg(unsigned char *board, t_proc *c_proc, int v)
{
	unsigned int		add;
	unsigned int		id;
	int					reg_nb;
	...
	add = ((c_proc->r[reg_nb2] + bit_cat(board, c_proc, id, 4) % IDX_MOD) + c_proc->i) % MEM_SIZE;
	if (v == 1)
		cmd_verbose_sti(board, c_proc, bit_cat(board, c_proc, id, 4), c_proc->r[reg_nb2]);
	sti_result(board, c_proc, reg_nb, add);
}

static void				add_dir_ind(unsigned char *board, t_proc *c_proc, int v)
{
	unsigned int		add;
	unsigned int		id;
	int					reg_nb;
	...
	reg_nb = board[(c_proc->i + 2) % MEM_SIZE] - 1;
	id = bit_cat(board, c_proc, 3, 2);
	add = ((bit_cat(board, c_proc, 5, 2) + bit_cat(board, c_proc, id, 4) % IDX_MOD) + c_proc->i) % MEM_SIZE;
	if (v == 1)
		cmd_verbose_sti(board, c_proc, bit_cat(board, c_proc, id, 4), bit_cat(board, c_proc, 5, 2));
	sti_result(board, c_proc, reg_nb, add);
}

static void				add_ind_ind(unsigned char *board, t_proc *c_proc, int v)
{
	unsigned int		add;
	int					reg_nb;
	...
	add = 0;
	reg_nb = 0;
	add = ((bit_cat(board, c_proc, 3, 2) + bit_cat(board, c_proc, 5, 2) % IDX_MOD) + c_proc->i) % MEM_SIZE;
	reg_nb = board[(c_proc->i + 2) % MEM_SIZE] - 1;
	if (v == 1)
...
static void				add_reg_ind(unsigned char *board, t_proc *c_proc, int v)
{
	unsigned int		add;
	int					reg_nb;
	int					reg_nb2;
	...
	add = ((c_proc->r[reg_nb2] + bit_cat(board, c_proc, 4, 2) % IDX_MOD) + c_proc->i) % MEM_SIZE;
	if (v == 1)
...
void					cmd_sti(unsigned char *board, t_proc *c_proc, t_cor *core)
{
	if (c_proc->ctp == 25)
	{
		if (board[(c_proc->i + 1) % MEM_SIZE] == 0x68)
			add_ind_ind(board, c_proc, core->options.verbose);
		else if (board[(c_proc->i + 1) % MEM_SIZE] == 0x58)
			add_reg_ind(board, c_proc, core->options.verbose);
		else if (board[(c_proc->i + 1) % MEM_SIZE] == 0x64)
			add_ind_reg(board, c_proc, core->options.verbose);
		else if (board[(c_proc->i + 1) % MEM_SIZE] == 0x54)
			add_reg_reg(board, c_proc, core->options.verbose);
		else if (board[(c_proc->i + 1) % MEM_SIZE] == 0x74)
			add_dir_reg(board, c_proc, core->options.verbose);
		else if (board[(c_proc->i + 1) % MEM_SIZE] == 0x78)
			add_dir_ind(board, c_proc, core->options.verbose);
